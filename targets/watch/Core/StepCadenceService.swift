...
final class StepCadenceService {
  ...
  func stop() { if let q = query { HKHealthStore().stop(q) }; query = nil; anchor = nil; ema = nil }
  ...
  private func consume(samples: [HKSample]?, newAnchor: HKQueryAnchor?) {
    ...
    var steps = 0.0; var earliest = Date.distantFuture; var latest = Date.distantPast
    for s in arr {
      steps += s.quantity.doubleValue(for: .count())
      earliest = min(earliest, s.startDate); latest = max(latest, s.endDate)
    }
